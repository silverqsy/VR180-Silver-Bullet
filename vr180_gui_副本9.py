    ...
    def _run_with_gopro_roll(self):
        ...
        try:
            ...
            def apply_lut_3d(frame, lut, intensity=1.0):
                ...
                .cube LUT format stores data with R varying fastest, then G, then B.
                So the data is indexed as lut[B, G, R] to get the output RGB value.
                """
                ...
                # Convert to float (frame is BGR from OpenCV)
                img = frame.astype(np.float32) / 255.0

                # Extract BGR channels and compute indices
                ...
                # Trilinear interpolation - index as [B, G, R] for .cube format
                c000 = lut[b0, g0, r0]
                c001 = lut[b0, g0, r1]
                c010 = lut[b0, g1, r0]
                c011 = lut[b0, g1, r1]
                ...
                result_rgb = c0 * (1 - fb) + c1 * fb

                # Blend with original based on intensity
                if intensity < 1.0:
                    img_rgb = img[:, :, ::-1]  # BGR to RGB for blending
                    result_rgb = img_rgb + (result_rgb - img_rgb) * intensity

                # RGB back to BGR for OpenCV
                result = result_rgb[:, :, ::-1]
            ...
            try:
                ...
                try:
                    _, stderr = encode_proc.communicate(timeout=60)
...
class VR180ProcessorGUI(QMainWindow):
    ...
    def _apply_preview_filters_to_cached_frame(self):
        ...
        if abs(lift) > 0.01 or abs(gamma - 1.0) > 0.01 or abs(gain - 1.0) > 0.01:
            ...
            if not hasattr(self, '_color_lut_cache') or self._color_lut_cache_key != color_key:
