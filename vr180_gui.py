...
def get_hw_encoder_args(codec: str, encoder_type: str, quality: int, bitrate: int, use_bitrate: bool, bit_depth: int = 8):
    ...
    if encoder_type == 'nvenc':
        ...
        if use_bitrate:
            enc = ["-c:v", hw_encoder, "-preset", "p4", "-rc", "vbr", "-b:v", f"{bitrate}M"]
        else:
            enc = ["-c:v", hw_encoder, "-preset", "p4", "-rc", "vbr", "-cq", str(quality), "-b:v", "0"]
        if codec == 'h265':
            enc.extend(["-tag:v", "hvc1"])
            if bit_depth == 10:
                enc.extend(["-pix_fmt", "p010le"])
    ...
    def _run_with_gopro_roll(self):
        ...
        try:
            ...
            def apply_lut_3d_fast(frame, lut, intensity):
                """Apply 3D LUT at half resolution for speed."""
                if lut is None or intensity < 0.01:
                    return frame
                h, w = frame.shape[:2]
                lut_size = lut.shape[0]

                # Downscale 2x for LUT (4x fewer pixels to interpolate)
            ...
            def process_frame_opencv(frame, left_roll, right_roll, global_shift,
                ...
                result = apply_color_1d(result, color_1d_lut)
            ...
            self.status.emit("Processing frames...")
            ...
            encode_queue = queue.Queue(maxsize=4)   # processed frames waiting to be encoded
            pipeline_error = [None]  # shared error state

            def decode_thread():
                """Read raw frames from FFmpeg decoder."""
                try:
            ...
            try:
                ...
                self.progress.emit(100)
                self.finished_signal.emit(True, f"Complete! (GoPro roll fix: {frame_count} frames processed)")
...
class VR180ProcessorGUI(QMainWindow):
    ...
    def _apply_preview_filters_to_cached_frame(self):
        ...
        # Combine back - swap order to match expected output format
        result = np.hstack([right, left])
