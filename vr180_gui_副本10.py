...
class VideoProcessor(QThread):
    ...
    def _run_with_gopro_roll(self):
        ...
        try:
            ...
            # Check if we have any base yaw/pitch adjustments (these still need v360)
            has_yaw_pitch = any([abs(left_yaw) > 0.01, abs(left_pitch) > 0.01,
                                abs(right_yaw) > 0.01, abs(right_pitch) > 0.01])

            if has_yaw_pitch:
                self.status.emit("Note: Yaw/pitch adjustments present - using hybrid FFmpeg/OpenCV processing")
            ...
            def apply_lut_3d(frame, lut, intensity=1.0):
                ...
                # Convert to float and scale to LUT indices
                img = frame.astype(np.float32) / 255.0

                # BGR to RGB for LUT (most LUTs expect RGB)
                img = img[:, :, ::-1]

                # Trilinear interpolation
                indices = img * (lut_size - 1)
                ...
                if intensity < 1.0:
                    result = img + (result - img) * intensity
            ...
            def process_frame_opencv(frame, left_roll, right_roll):
                """Process a single SBS frame using OpenCV.

                Note: Global shift and yaw/pitch adjustments are applied in FFmpeg decode pipeline.
                This function only handles roll (which varies per-frame for GoPro fix) and color/LUT.
                """
            ...
            # Build filter chain for yaw/pitch adjustments and global shift
            decode_filters = []
            ...
            if shift != 0:
                if shift > 0:
                    decode_filters.extend([
                        f"{current_label}split=2[sh_a][sh_b]",
                        f"[sh_a]crop={shift}:ih:0:0[sh_right]",
                        f"[sh_b]crop=iw-{shift}:ih:{shift}:0[sh_left]",
                        f"[sh_left][sh_right]hstack=inputs=2[shifted]"
                    ])
                else:
                    abs_shift = abs(shift)
                    decode_filters.extend([
                        f"{current_label}split=2[sh_a][sh_b]",
                        f"[sh_a]crop={abs_shift}:ih:iw-{abs_shift}:0[sh_left]",
                        f"[sh_b]crop=iw-{abs_shift}:ih:0:0[sh_right]",
                        f"[sh_left][sh_right]hstack=inputs=2[shifted]"
                    ])
                current_label = "[shifted]"

            # Split into left and right for yaw/pitch adjustments
            if has_yaw_pitch:
                decode_filters.extend([
                    f"{current_label}split=2[full1][full2]",
                    "[full1]crop=iw/2:ih:0:0[left_in]",
                    "[full2]crop=iw/2:ih:iw/2:0[right_in]"
                ])

                # Apply yaw/pitch via v360 (roll will be applied via OpenCV)
                if abs(left_yaw) > 0.01 or abs(left_pitch) > 0.01:
                    decode_filters.append(f"[left_in]v360=input=hequirect:output=hequirect:yaw={left_yaw}:pitch={left_pitch}:roll=0:interp=lanczos[left_out]")
                else:
                    decode_filters.append("[left_in]null[left_out]")

                if abs(right_yaw) > 0.01 or abs(right_pitch) > 0.01:
                    decode_filters.append(f"[right_in]v360=input=hequirect:output=hequirect:yaw={right_yaw}:pitch={right_pitch}:roll=0:interp=lanczos[right_out]")
                else:
                    decode_filters.append("[right_in]null[right_out]")

                # Combine back
                decode_filters.append("[left_out][right_out]hstack=inputs=2[out]")
                current_label = "[out]"

            # Build decode command
            if decode_filters:
                filter_complex = ";".join(decode_filters)
                # If we have filters, output from the last label
                output_label = current_label.strip("[]") if current_label != "[0:v]" else "0:v"
                decode_cmd = [
                    get_ffmpeg_path(),
                    "-ss", str(start_time),
                    "-t", str(duration),
                    ...
                    "-map", f"[{output_label}]" if output_label != "0:v" else "0:v",
                    "-f", "rawvideo",
                    "-pix_fmt", "bgr24",
                    "-v", "quiet",
                    "-"
            ...
            if output_codec == "h265":
                if cfg.use_hardware_accel and sys.platform == 'darwin':
                    ...
                    if cfg.use_bitrate:
                        enc = ["-c:v", "libx265", "-b:v", f"{cfg.bitrate}M", "-preset", "fast", "-tag:v", "hvc1"]
                    else:
                        enc = ["-c:v", "libx265", "-crf", str(cfg.quality), "-preset", "fast", "-tag:v", "hvc1"]
            elif output_codec == "prores":
                profile_map = {"proxy": "0", "lt": "1", "standard": "2", "hq": "3", "4444": "4", "4444xq": "5"}
                if cfg.use_hardware_accel and sys.platform == 'darwin':
                    enc = ["-c:v", "prores_videotoolbox", "-profile:v", profile_map.get(cfg.prores_profile, "3")]
                ...
                if cfg.use_bitrate:
                    enc = ["-c:v", "libx264", "-b:v", f"{cfg.bitrate}M", "-preset", "fast"]
                else:
                    enc = ["-c:v", "libx264", "-crf", str(cfg.quality), "-preset", "fast"]
            ...
            encode_proc = subprocess.Popen(encode_cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE,
            ...
            try:
                while True:
                    ...
                    # Process frame with OpenCV (global shift and yaw/pitch applied in FFmpeg decode)
                    processed = process_frame_opencv(frame, left_roll, right_roll)
                    ...
                    if frame_count - last_update >= 10:
                        progress = int((frame_count / total_frames) * 90)
                        self.progress.emit(min(progress, 90))
                        self.status.emit(f"Frame {frame_count}/{total_frames} (roll: {gopro_roll:.1f}°, L={left_roll:.1f}° R={right_roll:.1f}°)")
                ...
                if encode_proc.returncode != 0:
                    stderr = encode_proc.stderr.read().decode() if encode_proc.stderr else ""
                    raise Exception(f"Encoding failed: {stderr[-500:]}")
