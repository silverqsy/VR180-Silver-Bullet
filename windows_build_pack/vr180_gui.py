...
class VideoProcessor(QThread):
    ...
    def _convert_to_mvhevc(self, cfg: ProcessingConfig):
        """Convert to MV-HEVC with APMP metadata for Vision Pro (re-encodes)"""
        import tempfile
        import shutil
        ...
        try:
            # Use the bitrate from H.265 settings
            # If using CRF, convert to estimated bitrate (higher quality = higher bitrate)
            if cfg.use_bitrate:
                bitrate_mbps = cfg.bitrate
            else:
                # Estimate bitrate from CRF (lower CRF = higher bitrate)
            ...
            # Determine appropriate MV-HEVC preset based on resolution
            # VR180 videos are typically 4K or higher
            preset = "PresetMVHEVC4320x4320"  # Default for high quality

            # Build avconvert command with bitrate
            # Note: avconvert doesn't support direct bitrate control, so we use preset
            # For better quality control, we could use FFmpeg instead, but avconvert
            # handles APMP metadata automatically
            cmd = [
                'avconvert',
                '--source', str(cfg.output_path),
                '--output', str(temp_file),
                '--preset', preset,
                '--sourceProjection', 'HalfEquirectangular',  # VR180 projection
                '--sourceViewPacking', 'SideBySide',  # Stereo left/right
                '--progress',
                '--replace'
            ]

            self.status.emit(f"Encoding MV-HEVC at ~{bitrate_mbps}Mbps (this may take a while)...")

            # Run avconvert with progress monitoring
            ...
            for line in process.stdout:
                # avconvert outputs progress like "Progress: 45%"
                if 'Progress:' in line:
                    try:
                        pct = line.split('Progress:')[1].strip().rstrip('%')
                        progress = int(float(pct))
                        self.status.emit(f"MV-HEVC encoding: {progress}%")
                    except:
                        pass
            ...
            if process.returncode != 0:
                raise Exception(f"MV-HEVC conversion failed with code {process.returncode}")
