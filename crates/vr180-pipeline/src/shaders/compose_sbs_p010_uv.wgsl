// SBS RGBA16 → P010 UV plane.
//
// One thread per output UV pixel (which covers a 2×2 block of the
// full-res image). Averages the 2×2 block, computes Cb and Cr in
// 10-bit video range, writes interleaved U V to Rg16Unorm at the
// top 10 bits.
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // This UV pixel covers full-res block (2*gid.x, 2*gid.y) to
    // (2*gid.x+1, 2*gid.y+1). For each of those 4 source pixels,
    // figure out left/right and read.
    let full_w = uv_dim.x * 2u;
    let full_h = uv_dim.y * 2u;
    let eye_w = u.eye_w;
